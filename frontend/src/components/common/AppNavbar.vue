  ...
  <nav class="navbar" :class="{ scrolled: isScrolled }">
    <div class="container nav-inner">
      ...
      <div class="nav-links">
        ...
        <RouterLink v-if="auth.isAdmin || auth.isDeveloper" to="/events/add" class="btn btn-ghost btn-sm">+ Event</RouterLink>
        <RouterLink v-if="auth.isAdmin || auth.isDeveloper" to="/movies/add" class="btn btn-ghost btn-sm">+ Movie</RouterLink>
      </div>

      <div class="nav-actions">
        <template v-if="auth.isLoggedIn">
          <RouterLink to="/profile" class="user-info">
            <span class="badge" :class="auth.isAdmin ? 'badge-admin' : 'badge-user'">{{ auth.user.role }}</span>
            {{ auth.user?.name }}
          </RouterLink>
    ...
    <div class="mobile-menu" :class="{ open: menuOpen }" @click="menuOpen = false">
      ...
      <RouterLink v-if="auth.isAdmin || auth.isDeveloper" to="/events/add">+ Event</RouterLink>
      <RouterLink v-if="auth.isAdmin || auth.isDeveloper" to="/movies/add">+ Movie</RouterLink>
      <RouterLink v-if="auth.isAdmin || auth.isDeveloper" to="/stats">Stats</RouterLink>
      <template v-if="auth.isLoggedIn">
...
.logo-text { font-family: var(--font-display); font-size: 1.6rem; letter-spacing: 0.08em; }
...
.user-info { display: flex; align-items: center; gap: 0.5rem; font-size: 0.85rem; color: var(--text-muted); }
...
.mobile-menu { display: none; flex-direction: column; gap: 0.5rem; padding: 1rem 2rem; background: var(--bg-card); border-top: 1px solid var(--border); }
.mobile-menu a, .mobile-menu button { padding: 0.6rem 0; color: var(--text); font-size: 1rem; background: none; border: none; cursor: pointer; text-align: left; border-bottom: 1px solid var(--border); }
